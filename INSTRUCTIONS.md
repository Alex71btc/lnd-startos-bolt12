...
## 🔄 Migrating from Start9 LND (IMPORTANT)

If you already have an existing Start9 LND node, you can migrate it into this node (lndbolt).
This will enable custom onion messages-protocol on your LND in order to make Bolt12 Offer funktionality work with Bolt 12 Pay.
...
⚠️ Important:
- Always stop both nodes before importing
- Never run two nodes with the same state at the same time
- Do NOT run original LND and lndbolt in parallel with the same data

### How to Import

1. Stop:
   - lnd (official Start9 LND)
   - lndbolt

2. Open lndbolt service menu
...
## ⚡ Quick Start: Connect with Zeus (Recommended)

You can immediately control your node using Zeus Wallet.

### Steps

1. Open Start9 UI → your LND or lndbolt service  
2. Go to **Properties → LND REST Connect**  
3. Scan the QR code with Zeus  

### ⚠️ Important Fix

After scanning, edit the connection in Zeus:

- Replace host with your **.onion address** (from Start9 UI)
- Change:
  - ❌ http://  
  - ✅ https://  
...
Correct (working):
https://your-node-address.onion:8080

---

### 💡 Why this matters

- Internal IPs often fail
- Zeus requires proper TLS (https)
- Tor (.onion) provides stable access
...
- Never run two LND nodes with the same wallet state at the same time
...
1. Run `sudo podman exec -ti lndbolt.embassy lncli --rpcserver=lndbolt.embassy wtclient towers`
